...
GETTING STARTED
  The canonical install is the act plugin — it bundles this binary,
  the workflow skill, and the MCP server:

    /plugin marketplace add aac/act
    /plugin install act@act

  If you want just the binary (no plugin), go install bootstraps it:
  ...
  Other binary-only alternates (brew tap, prebuilt release binaries)
  are tracked separately and are not the recommended path for new
...
THE CANONICAL WORK LOOP (use this in every session)
  ...
  Prefer the composed verbs when you can: 'act next' bundles steps
  1-2 (ready + claim + show) and 'act finish' is 'act close' (step 4):
  it commits the close op and pushes THAT op to the .act/ tracker
  remote. It does NOT push your host work commit (the one carrying the
  'Act-Id:' trailer from step 5) — you still 'git push' your code
  yourself (step 6), or the orchestrator's merge publishes it. So a
  session drives the loop with 'act next' / work / commit / 'act
  finish' / 'git push'. The composed verbs mirror the act_next /
  act_finish MCP tools, so both interfaces offer the same flow.
...
THE LOOP IN DETAIL
  ...
  Closing
    ...
    With an origin configured on the nested .act/ repo, the close (and
    'act finish') pushes that commit itself — there is no separate
    'git push' step for the close op. With no origin remote, close and
    'act finish' commit locally and skip the push with no network: the
    close still succeeds (exit 0) and nothing reaches the wire.
...
EXTERNAL DEPS
  ...
  a Jira card, anything with its own canonical id. Attach an opaque ref
  as a blocking external dep with 'act dep add --external' — the same
  verb as internal blockers, so "blocked by X" is one command whether X
  is an act id or an external ref:

    $ act dep add <id> --external "linear:ENG-123"
    $ act dep add <id> --external "gh:org/other-repo#42" --external "..."

  Each --external writes one add_external_dep op. The ref is stored
  ...
  'act update --ext-rm' (also idempotent on absence, so an orchestrator
  can fire the clear twice without erroring):

    $ act update <id> --ext-rm "linear:ENG-123"

  The surfaces are symmetric with internal blockers: 'act dep add' adds
  the edge (--blocked-by for an act id, --external for a cross-tracker
  ref); 'act update' removes it (--dep-rm for internal, --ext-rm for
  external). An issue may carry both kinds; either keeps it out of
  'act ready' until cleared.
...
ERROR CODES (STABLE; RENAMING IS A BREAKING CHANGE)
  ...
  Spec canonical codes (per spec.md §error-envelope):
...
AN OP WHOSE COMMIT FAILED IS INVISIBLE
  ...
  as '(op file preserved at <path>; nothing was recorded)'. Copy it
