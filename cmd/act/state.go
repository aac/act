...
// stateExportHelpText is the --help text for `act state export`. It is a
// package-level constant so the TestDocClaim_* tests can assert the
// load-bearing strings without executing the binary.
//
// The claims that docs_sweep_test.go registry entries pin:
//
//	--json envelope success: harvested_ops is a JSON array of op-path strings; exit 0
//	--json envelope error:   worker_state_not_found error code; exit 2
const stateExportHelpText = `act state export <dir> [--dry-run] [--json]

Pull new ops from <dir>/.act/ops/ back into this repo's .act/, commit them
on the nested .act/.git, and re-fold the index. Does NOT push — that is the
caller's responsibility. Directory-scoped: act does not care what <dir> is for.

--json envelope (success, exit 0):
  {
    "harvested_ops":    [...],   // JSON array of op-path strings relative to .act/ops/
                                 // that were newly copied this run (or would be, on --dry-run)
    "skipped_ops":      [...],   // [{path, reason}] for source ops already present
    "fold_diff_summary": {...},  // {issues_indexed, ops_added} from the index rebuild
    "dry_run":          false
  }

--json envelope (error — <dir> has no .act/, exit 2):
  {"error":"worker_state_not_found","message":"...","details":{}}

Exit codes:
  0  success (including zero-op no-op runs)
  2  <dir> has no .act/ — pass a path seeded by 'act state import' (worker_state_not_found)
...
func runStateExport(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), stateExportHelpText)
		fs.PrintDefaults()
	}
