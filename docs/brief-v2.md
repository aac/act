...
**Redact / delete.** A `redact` op overwrites prior op payload contents in derived snapshots while leaving a tombstone in the op log. For true secret removal from git history, the documented escape hatch is `git-filter-repo`. Policy is stated, not silent.
...
- `act doctor [--check <name>]` — battery of checks: `orphan-close`, `orphan-ops`, `dangling-deps`, `time-travel`, `cycle`, `unknown-op-version`, `index-divergence`. Default runs all.
...
Default behavior is auto-commit per op with an `act-op: <id> <op-type>` message prefix. Durability beats history aesthetics. `--no-commit` is available for batching agents. Before push, `act` collapses contiguous `act-op:` commits into a single squashed commit so the visible history isn't drowned in tracker noise.
...
- Two agents writing logically conflicting ops to the same field: both files exist. Fold applies HLC order; ties broken by op-hash. Determinism is preserved.
- `--claim` is blocking-and-verifying. The protocol is: (1) write the claim op, (2) `git pull --rebase` from the configured remote, (3) re-fold the issue, (4) report win/loss, (5) push only on win unless `--push-always`. On loss, exit non-zero with structured `{"claimed": false, "winner": "..."}`. `--wait` polls to stability; `--isolated` opts out of remote fetch for offline use.
...
`.act/hooks/{post-create,post-close,post-claim}` are plain executables run synchronously after the corresponding op is committed. That's the entire framework in v1. Pubsub and event-bus stay deferred.

## MCP server

- `act mcp` exposes the CLI surface as MCP tools
- 1:1 tools for each command, plus three composed convenience tools: `act_next` (ready + claim + show), `act_finish` (close + commit-marker), `act_block` (status=blocked + add-dep). Tool descriptions mark the composed tools as the recommended path.
- Stdio transport; no network ports
- Cowork plugin manifest references the binary path; user installs `act` and adds the plugin

## Bootstrap

A 50-line shell script writes `_generated/projects/act/issues.jsonl` — the simplest possible op-log. v0.1 of `act` ships an importer that consumes that JSONL on first run, so the op-log primitive is dogfooded from line one and the project tracks itself before the binary is finished.
