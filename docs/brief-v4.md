...
Default behavior is auto-commit per op with an `act-op: <id> <op-type>` message prefix. Durability beats history aesthetics. Op-commits run with `git commit --no-verify` to bypass host repo pre-commit and commit-msg hooks (op commits touch only `.act/ops/**` and are validated by `act doctor`; `--no-verify` is the cross-platform way to skip these, including on Windows); `--verify` opts back into host hooks. `--no-commit` is available for batching agents. `act` does NOT push by default — auto-commit yes, auto-push no. `--push` is opt-in per command. `--isolated` shares the same code path: commit locally, no network. Before push, `act` collapses contiguous `act-op:` commits into a single squashed commit so the visible history isn't drowned in tracker noise. `act` refuses to squash a range whose op payloads carry a `writer_version` newer than the running binary (force-upgrade path); the squash commit message carries `act-squash: writer_version=<max-of-range>` so downstream readers and `act doctor` can correlate.
...
- `act doctor` scans both commit messages and the diff of `.act/ops/**` for close-ops; any commit touching an issue's ops directory is evidence. Snapshots carry a `closed_by_tree` reverse index — the git tree hash of `.act/ops/<id>/` at close time — for symmetric verification. Tree hashes are invariant under commit squash and rebase, so the check survives both.
...
`.act/hooks/{post-create,post-close,post-claim}` are plain executables run synchronously after the op file is staged but before the op-commit. Hooks receive the op JSON on stdin and `ACT_OP_ID`, `ACT_OP_TYPE`, `ACT_ISSUE_ID`, `ACT_HOOK_PHASE=pre-commit-op` env vars. Default 5s timeout (SIGTERM then SIGKILL); non-zero exit aborts the commit and unstages the op file. Hooks NEVER run during fold, replay, import, or clone — only on the writer that originally produced the op. That's the entire framework in v1. Pubsub and event-bus stay deferred.
...
A 50-line shell script writes `_generated/projects/act/issues.jsonl` — the simplest possible op-log. Each line is one op object using the v0.1 envelope: `{op_version, schema_version, op_type, issue_id, payload, hlc, node_id}`. v0.1 of `act` ships an importer that validates each line, replays it as if locally generated (assigning fresh HLCs and filenames), and emits an id-mapping file at `.act/imports/<iso-timestamp>.json` of the form `{"source": "issues.jsonl@<sha>", "mapping": {"bootstrap-id-1": "act-a1b2...", ...}}`, committed alongside the imported ops. Re-runs of the importer are idempotent and skip already-mapped source ids. `act show` and `act log` accept either pre- or post-import ids and resolve via the mapping files. The op-log primitive is dogfooded from line one and the project tracks itself before the binary is finished.
