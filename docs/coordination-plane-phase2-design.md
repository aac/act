...
**Status:** v4, plan-ready.
**Author:** drafted in session 2026-05-18; revised through two full review gates and one narrow remediation pass.
**Supersedes:** v3 (commit `047eb4d`), v2 (commit `4776643`), v1 (commit `00c4215`).
**v4 changes** (closes the narrow v3-remediation-check findings): `act remote sync` trigger mechanism committed (post-receive hook on orchestrator's `.act/.git` + write-path trigger on orchestrator's own writes); `--depth 1` shallow-rebase fallback specified (`git fetch --unshallow` on shallow-related rebase failure); doctor "slow write" check reframed as a write-time warning emitted by `actGitOps`; doctor gains an "upstream divergence" check so fail-soft sync's silent degradation eventually surfaces.
...
**Optional GitHub upstream** for durability and cross-machine convenience: the orchestrator's `.act/.git` has GitHub configured as a second remote (e.g., `origin-upstream`). State replication is by an explicit `act remote sync` call. **Not** a post-commit hook on worker clones (would fire mid-rebase during contention retries, publishing partial state). **Not** cron (multi-minute durability gap defeats the "as durable as the upstream" claim). Workers do not interact with the upstream directly — they only talk to the orchestrator's `.act/.git`. If the orchestrator's machine is gone, recovery is "clone the GitHub upstream, reconfigure as the new orchestrator."

**`act remote sync` triggers — two points, both on the orchestrator:**

1. **Post-receive hook on the orchestrator's `.act/.git`.** Installed by `act remote enable`. When a worker push successfully lands on the orchestrator (ref update completes — not mid-transfer, not under retry), git fires the hook, which detaches and runs `act remote sync` in the background. This dodges the original "no hooks" objection: post-receive fires only on successfully landed refs, never on partial state, never mid-rebase. The hook runs on the orchestrator (one machine), not on every worker clone.
2. **`actGitOps` write path on the orchestrator.** When the orchestrator runs its own `act` command (orchestrator's main checkout writing directly), the write path triggers a background `act remote sync` after its commit succeeds. Symmetrical to the post-receive case but for local writes that don't pass through the push interface.

Both triggers spawn `act remote sync` as a detached process and return immediately; the calling write path is not blocked. Sync is **fail-soft**: a failed upstream push logs to `.act/.sync-log` and the next successful sync clears the queue. Operators don't see warnings inline.

Doctor surfaces accumulated drift: see "Doctor in the three-state world" case (h) below — an "upstream is N commits behind local" check that fires when the gap exceeds a threshold (default 50 commits or 1 hour, whichever first), ensuring the fail-soft path's silent degradation eventually becomes visible without inline blocking.
...
**Slow-write warning (write-time, not doctor-time).** `actGitOps` measures wall-clock between op-file stage and commit completion. If the duration exceeds `act.slowWriteThresholdMs` (default 1000ms), it emits a warning to stderr at write time and appends a structured entry to `.act/.slow-writes` (rolling, capped at the last 100 entries). Doctor's remote-status block surfaces a count of slow writes in the last hour from this file — it does not retroactively measure past writes (which it cannot — the timing is observable only at write time). A persistently slow filesystem (NFS, encrypted overlay, FUSE mount) becomes visible as both inline warnings and a doctor-level summary.
...
**Shallow-rebase fallback.** The push-contention rebase loop may fail when the remote has advanced beyond the worker's shallow clone depth. Standard git servers auto-deepen on fetch, so most contention retries succeed without intervention. When auto-deepen is unavailable (constrained network, restrictive server config) and rebase fails with a shallow-related error, the retry loop runs `git fetch --unshallow origin <branch>` once before retrying step 2. After unshallowing, subsequent retries operate on full history. `--depth 1` is therefore best-effort: it's the fast path; full clone is the fallback.
...
| (new — Phase 2) | (h) optional upstream is N commits behind orchestrator → flag upstream-drift, suggest `act remote sync`. Detection: after a successful upstream fetch, `git rev-list --count origin-upstream/<branch>..origin/<branch>`. Threshold: `act.upstreamDriftThresholdCommits` (default 50) or `act.upstreamDriftThresholdSeconds` (default 3600), whichever exceeds first. Only fires if an `origin-upstream` remote is configured. |
