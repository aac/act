...
**Mode B — act is the session only.** An external orchestrator (the session-orchestrator project, or any equivalent) decides which issues are worked when, by whom, in what environment. Each agent invocation handles one issue and exits. The orchestrator owns lifecycle, parallelism, retry, cross-repo coordination, and environment provisioning. This is what `/act:work <id>` should drive.
