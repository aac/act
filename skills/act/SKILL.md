...
**If your worktree was created during dispatch and has no `.act/` yet, bootstrap it yourself with `act bootstrap-worker --from-cwd <orchestrator-path>`** (run from inside your worktree; target defaults to your cwd). Never `cp -r <orchestrator>/.act .` by hand: that copies the orchestrator's live `index.db`, which the orchestrator may have open, and the resulting stale/locked index causes silent op-write loss — your `act create` returns success but the op never lands where the orchestrator's `act harvest` can see it. The `--from-cwd` mode copies only the op log + config and rebuilds the index locally, which is always correct.
