...
// callBlock implements the `act_block` composed tool: writes an
// `add_dep type=blocks` op in a single git commit per spec §5.D.2.
// Note: a `update_field status=blocked` op is NOT written — blocked status
// is DERIVED from open blocked-by dep edges; the add_dep op alone is
// sufficient and the update_field op was a fold no-op.
...
func (s *Server) callBlockWithGops(raw json.RawMessage, factory gopsFactory) (any, bool) {
	...
	// Only op: add_dep type=blocks (full → parentFull). Blocked status is
	// DERIVED from the dep edge, so no update_field status=blocked op is
	// needed — that was a fold no-op (see bug act-018cb3).
	...
	envs := []op.Envelope{envDep}
	bodies := [][]byte{depEnvBody}
	opts := cli.WriteOpts{
		NoCommit: args.NoCommit,
		Push:     args.Push,
		Isolated: args.Isolated,
