...
// (see docs/coordination-plane-design.md, Phase 2 design). Copies the host repo's `.act/`
...
type BootstrapWorkerOptions struct {
	...
	// fragile interaction behind the concurrent-worker silent-data-loss
	...
	// user-visible error prose carries no worktree vocabulary.
	CmdName string
}
...
func RunBootstrapWorker(opts BootstrapWorkerOptions) (any, int) {
	...
	// invariant the nested-repo design asserts. Without it the copy would
	// silently lose the op history.
	srcGitPath := filepath.Join(srcAct, ".git")
	if _, err := os.Stat(srcGitPath); err != nil {
		return map[string]any{
			"error":   "act_state_not_initialized",
			"message": fmt.Sprintf(cmd+": source %s missing nested .git — run `act init`", srcAct),
		}, 3
	}
...
// interaction behind the concurrent-worker silent-data-loss bug.
...
// index locally from the copied op log. This closes the concurrent-worker
...
func runBootstrapFromCWD(opts BootstrapWorkerOptions) (any, int) {
	...
	if _, err := os.Stat(filepath.Join(srcAct, ".git")); err != nil {
		return map[string]any{
			"error":   "act_state_not_initialized",
			"message": fmt.Sprintf(cmd+": --from-cwd source %s missing nested .git — run `act init`", srcAct),
		}, 3
	}
