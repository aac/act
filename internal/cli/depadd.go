...
type DepAddResult struct {
	...
	// Summary is a plain-English rendering of the edge in its natural
	// direction (e.g. "act-a1b2 is blocked by act-9c2b"). The raw
	// child/parent fields read backwards as subject-verb-object for
	// edge_type=blocks (the child is blocked BY the parent, not the
	// reverse); summary exists so MCP/JSON callers have an unambiguous
	// direction signal without re-deriving it. See depDirectionPhrase.
	Summary   string `json:"summary"`
	Committed bool   `json:"committed"`
}

// depAddSummary renders the (child, parent, edge_type) triple in its
// natural-English direction, matching FormatDepAddHuman's verb choice.
func depAddSummary(child, parent, edgeType string) string {
	return fmt.Sprintf("%s %s %s", child, depDirectionPhrase(edgeType), parent)
}
...
func RunDepAdd(repoRoot string, opts DepAddOptions) (output any, exitCode int) {
		...
		for _, d := range r.Deps {
			if d.Parent == parentFull && d.EdgeType == edgeType {
				return DepAddResult{
					...
					Parent:    parentFull,
					EdgeType:  edgeType,
					Summary:   depAddSummary(childFull, parentFull, edgeType),
					Committed: false,
				}, 0
			}
		}
	...
	if werr != nil {
		...
		if msg, details, isTimeout := WriteLockTimeoutDetails(werr); isTimeout {
			return DepAddErrorOutput{Error: ErrWriteLockTimeout, Message: msg, Details: details}, 1
		}
...
// with spec.md §1 universal error table. The discrepancy between exit-2
// per the old per-section text (spec.md line 529) and exit-3 per the
...
func FormatDepAddHuman(res DepAddResult) string {
	summary := depAddSummary(res.Child, res.Parent, res.EdgeType)
	if !res.Committed {
		return fmt.Sprintf("Dep already present: %s\n", summary)
	}
