...
//
// This test exercises the same ldflag plumbing the release CI uses
// (-X github.com/aac/act/internal/version.Binary=...) so a refactor
// that breaks the stamp symbol, drops the module path, or makes
// version.Binary non-overridable is caught locally before a release.
...
func TestVersionEmbedsBuildVersion(t *testing.T) {
	...
	ldflags := "-X github.com/aac/act/internal/version.Binary=" + want
