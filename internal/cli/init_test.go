...
// TestDocClaim_InitCloseHookSample pins the SKILL.md "Pre-close gates" claim
// that `act init` drops an inert `.act/hooks/close.sample` scaffold, and
// asserts the behavior at the user-visible boundary: init writes the sample,
// commits it into the nested repo (so it travels to worktree workers), and
// creates no *active* `.act/hooks/close` (the .sample can't fire — the gate is
// opt-in, activated by the agent per project). act-119309.
func TestDocClaim_InitCloseHookSample(t *testing.T) {
	root := repoRootForDocClaim(t)

	// (1) Doc surface: the skill documents the scaffold by name.
	skill, err := os.ReadFile(filepath.Join(root, "skills/act/SKILL.md"))
	if err != nil {
		t.Fatalf("read SKILL.md: %v", err)
	}
	if !strings.Contains(string(skill), ".act/hooks/close.sample") {
		t.Errorf("SKILL.md no longer documents the .act/hooks/close.sample scaffold")
	}
	...
	body, err := os.ReadFile(filepath.Join(hooksDir, "close.sample"))
	if err != nil {
		t.Fatalf("expected scaffold at %s/close.sample: %v", hooksDir, err)
	}
	if len(body) == 0 {
		t.Errorf("close.sample is empty")
	}

	// (3) Inert: no *active* close hook is created — only the .sample, which
	// ResolveHook never executes. The gate is opt-in.
	if _, err := os.Stat(filepath.Join(hooksDir, "close")); !os.IsNotExist(err) {
		t.Errorf("init created an active .act/hooks/close; expected only close.sample (err=%v)", err)
	}

	// (4) Committed into the nested repo, so it reaches worktree workers.
	if tracked := mustGitOutput(t, config.Layout(repo).Root, "ls-files", "hooks/close.sample"); tracked == "" {
		t.Errorf("close.sample not committed in the nested .act repo (won't reach workers)")
	}
}
