...
// BinaryVersion is the human-readable version of the act binary. It mirrors
// version.Binary, which is the single source of truth and the symbol the
// release linker stamps:
//
//	go build -ldflags "-X github.com/aac/act/internal/version.Binary=1.2.3" ...
//
// It defaults to "dev" for local builds; CI stamps the real release version
// into internal/version.Binary at release time (commit-to-main model). Do NOT
// stamp internal/cli.BinaryVersion directly — this initializer is non-constant,
// so a -X on this symbol would be re-overwritten at init.
...
func RunVersion(checkRepo bool, repoRoot string) (output any, exitCode int) {
	...
	// Skew is an op-FORMAT mismatch, not a release-version one: compare the max
	// op writer_version against this binary's WriterVersion (the op format it
	// understands), not BinaryVersion (the release version, which is independent
	// of the op format and overridden by the release linker). They were equal
	// while both were "0.1.0", which masked this conflation.
	if maxVer != "" && compareSemver(maxVer, WriterVersion) > 0 {
		return map[string]any{
			"error":   "version_skew",
			"message": fmt.Sprintf("repo writer_version %s exceeds this binary's writer_version %s; upgrade act", maxVer, WriterVersion),
