...
// docs/spec.md) at the boundary an agent would actually hit. Internal
...
// (usage error) per spec.md's universal exit-code table.
//
...
// `go install github.com/aac/act/cmd/act@latest` is the documented
// binary-only bootstrap (the plugin install is the canonical path).
// The literal string lives in README.md and `act help`; a rename of
// the import path would change one place and miss the other silently.
// This asserts both surfaces.
func TestDocClaim_GoInstallPath(t *testing.T) {
	// The go-install path is documented in `act help` (the agent-facing
	// command reference), not the README's new-adopter install list — the
	// README bullet was removed in act-2645c7. Assert it stays in help.
...
// TestDocClaim_DepAdd_SummaryDirection pins the act-59ab1a claim that
// `act dep add --json` emits a `summary` field rendering the edge in its
// natural direction ("A is blocked by B"), so MCP/JSON callers have an
// unambiguous direction signal that the raw child/parent fields (which
// read backwards as SVO for blocks) lack. The spec documents the field
// in docs/spec.md's dep-add JSON output.
func TestDocClaim_DepAdd_SummaryDirection(t *testing.T) {
	site := t.TempDir()
	runGit(t, site, "init", "-q", "-b", "main")
	configureSite(t, site, "doc@example.com", "doc")
	mustRunAct(t, site, 0, "init", "--json")
	...
	addOut, _ := mustRunAct(t, site, 0, "dep", "add", idA, idB, "--type", "blocks", "--json")
	var resp map[string]any
	if err := json.Unmarshal([]byte(addOut), &resp); err != nil {
		t.Fatalf("dep add --json parse: %v\n%s", err, addOut)
	}
	wantSummary := idA + " is blocked by " + idB
	if resp["summary"] != wantSummary {
		t.Errorf("dep add summary = %q, want %q", resp["summary"], wantSummary)
	}
	// The raw fields stay child=A, parent=B (child is the dependent).
	if resp["child"] != idA || resp["parent"] != idB {
		t.Errorf("dep add child/parent = %v/%v, want %s/%s", resp["child"], resp["parent"], idA, idB)
	}
}

// TestDocClaim_DepRm_NaturalDirectionNoArrow pins the act-03cd05 fix: the
// `act update --dep-rm` not-found message renders in natural English
// ("<child> is blocked by <parent>") instead of the legacy
// `<child> --[blocks]--> <parent>` arrow, which read as the inverse of the
// real semantic and misled direction probing.
func TestDocClaim_DepRm_NaturalDirectionNoArrow(t *testing.T) {
	site := t.TempDir()
	runGit(t, site, "init", "-q", "-b", "main")
	configureSite(t, site, "doc@example.com", "doc")
	mustRunAct(t, site, 0, "init", "--json")
	...
	// Real edge: A is blocked by B. Probe the NON-existent reverse direction
	// (B --dep-rm A) so we hit the dep_not_found message.
	mustRunAct(t, site, 0, "dep", "add", idA, idB, "--type", "blocks")
	rmStdout, rmStderr := mustRunAct(t, site, 1, "update", idB, "--dep-rm", idA)
	rmOut := rmStdout + rmStderr

	wantPhrase := idB + " is blocked by " + idA
	if !strings.Contains(rmOut, wantPhrase) {
		t.Errorf("dep-rm not-found message missing natural-direction phrase %q\ngot:\n%s", wantPhrase, rmOut)
	}
	if strings.Contains(rmOut, "--[blocks]-->") {
		t.Errorf("dep-rm not-found message still contains legacy inverted arrow `--[blocks]-->`:\n%s", rmOut)
	}
}
...
// boundary that spec.md §7.4 ("concurrent_claim_two_writers") and the
...
// made in docs/spec.md: `act show --json <id>` includes `blocked_by`
...
// in docs/spec.md: the dep-edge JSON shape emitted by `act show
...
// docs/spec.md and cmd/act/help.go: `act update --claim <id>` exits 2
...
// TestDocClaim_BlockedStatus_RequiresDepEdge pins the act-36b6d3 claim
// made in cmd/act/update.go's --status flag-help and in docs/spec.md:
// `act update --status blocked <id>` with no backing blocked-by dep edge
// exits 2 with envelope code `blocked_requires_dep` rather than reporting
// a false success (the prior bug: the op was silently discarded by fold
// because applyUpdateField ignores status updates).
//
// Two cases asserted at the subprocess boundary (same surface a cold-start
// agent hits):
//
//  1. No dep edge → exit 2, error blocked_requires_dep.
//
//  2. With a dep edge (added via `act dep add --blocked-by`) → exit 0,
//     the command succeeds (the dep makes the blocked status real via fold
//     dep-edge semantics).
func TestDocClaim_BlockedStatus_RequiresDepEdge(t *testing.T) {
	site := t.TempDir()
	runGit(t, site, "init", "-q", "-b", "main")
	configureSite(t, site, "doc@example.com", "doc")
	mustRunAct(t, site, 0, "init", "--json")

	outA, _ := mustRunAct(t, site, 0, "create", "blocked-status probe A", "--json")
	idA := pickIDFromJSON(t, outA)
	outB, _ := mustRunAct(t, site, 0, "create", "blocker B", "--json")
	idB := pickIDFromJSON(t, outB)

	// Case 1: no dep edge → exit 2 with blocked_requires_dep.
	noDepOut, _, noDepCode := runAct(t, site, "update", idA, "--status", "blocked", "--json")
	if noDepCode != 2 {
		t.Fatalf("--status blocked with no dep: exit %d, want 2; stdout:\n%s", noDepCode, noDepOut)
	}
	if !strings.Contains(noDepOut, `"error":"blocked_requires_dep"`) &&
		!strings.Contains(noDepOut, `"error": "blocked_requires_dep"`) {
		t.Errorf("--status blocked with no dep: stdout missing blocked_requires_dep code:\n%s", noDepOut)
	}

	// Case 2: with a dep edge → exit 0 (dep makes blocked status valid).
	mustRunAct(t, site, 0, "dep", "add", idA, idB, "--type", "blocks")
	withDepOut, _, withDepCode := runAct(t, site, "update", idA, "--status", "blocked", "--json")
	if withDepCode != 0 {
		t.Fatalf("--status blocked with dep: exit %d, want 0; stdout:\n%s", withDepCode, withDepOut)
	}
}

// TestDocClaim_BlockedStatus_FlagHelpMentionsDerived pins the act-36b6d3
// claim made in cmd/act/update.go's --status flag-help: the help text no
// longer advertises 'blocked' as a directly-settable status value. Instead
// it states that 'blocked' is derived from blocked-by dep edges and points
// agents to `act dep add --blocked-by`.
//
// Asserted at the `act update --help` stderr boundary; both the
// "derived from blocked-by" phrasing and the `--blocked-by` pointer must
// appear so an agent reading the flag-help learns the correct path.
func TestDocClaim_BlockedStatus_FlagHelpMentionsDerived(t *testing.T) {
	site := t.TempDir()
	runGit(t, site, "init", "-q", "-b", "main")
	configureSite(t, site, "doc@example.com", "doc")
	mustRunAct(t, site, 0, "init", "--json")

	_, stderr, _ := runAct(t, site, "update", "--help")
	help := stderr

	// The flag-help must mention that 'blocked' is derived, not directly settable.
	if !strings.Contains(help, "derived from blocked-by dep edges") {
		t.Errorf("act update --help: --status flag-help should state 'blocked' is derived; got:\n%s", help)
	}
	// Must point to `act dep add --blocked-by` as the correct path.
	if !strings.Contains(help, "--blocked-by") {
		t.Errorf("act update --help: --status flag-help should mention --blocked-by; got:\n%s", help)
	}
}

// TestDocClaim_BlockedStatus_SpecSaysDerived pins the act-15d5e9 spec
// correction: docs/spec.md now states that "blocked" is a DERIVED display
// state computed from open blocked-by dep edges, not a stored or directly
// settable status value. The test reads the spec file and asserts the
// corrected wording is present in both the data-model section (status enum
// comment) and the CRDT table row (status LWW rule).
//
// Drift shape: someone rewrites the status enum comment or the CRDT table
// row in a way that re-introduces "blocked" as a stored status — this test
// catches that by asserting the "never stored" framing is present verbatim.
func TestDocClaim_BlockedStatus_SpecSaysDerived(t *testing.T) {
	...
	// The status enum comment must state blocked is derived, not stored.
	if !strings.Contains(spec, `"blocked" is a DERIVED display state`) {
		t.Errorf("docs/spec.md status enum comment should state 'blocked' is a DERIVED display state; not found")
	}
	// The CRDT table row must state stored values are open, in_progress, closed only.
	if !strings.Contains(spec, "Stored values are `open`, `in_progress`, and `closed` only") {
		t.Errorf("docs/spec.md CRDT table should state stored values are open/in_progress/closed only; not found")
	}
	// Cross-reference to dep mechanism must be present.
	if !strings.Contains(spec, "act dep add --blocked-by") {
		t.Errorf("docs/spec.md should cross-reference 'act dep add --blocked-by'; not found")
	}
}
